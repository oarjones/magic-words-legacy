...
public class PanAndZoom : MonoBehaviour
{
    ...
    //public GameObject RunerewriteScroll;
    ...
    void Update()
    {
        //if (RuneRewriteController.runeRewiteActive)
        //{
        ...
        UpdateWithTouch();
            ...
            if(gameBoard.Initialized)
                gameBoard.SetPlayerIconPosition(GameActor.Player);
        }
        else
        {
    ...
    void UpdateWithMouse()
    {
        ...
        //{
        //    gameBoard.GetCurrentUserTile().neighbors.neighbor_TOP.Select();
        //}
    ...
    void UpdateWithTouch()
    {
        ...
        else if (touchCount == 2)
        {
            ...
            if (touch0.phase == UnityEngine.TouchPhase.Ended || touch1.phase == UnityEngine.TouchPhase.Ended) return;
    ...
    void OnSwipe(Vector2 deltaPosition)
    {
        ...
        {
            gameBoard.transform.position += (cam.ScreenToWorldPoint(deltaPosition) - cam.ScreenToWorldPoint(Vector2.zero));
        }
    }
    ...
    void OnPinch(Vector2 center, float oldDistance, float newDistance, Vector2 touchDelta)
    {
        ...
        if (controlCamera && cameraControlEnabled)
        {
            // Convertir el centro del pinch de espacio de pantalla a espacio mundial
            Vector3 pinchCenterWorld = cam.ScreenToWorldPoint(center);
            pinchCenterWorld.z = gameBoard.transform.position.z; // Asegurarse de que el z coincide con el del mapa

            // Cambiar el cálculo de scaleChange para invertir el comportamiento del zoom
            float scaleChange = newDistance / oldDistance; // Esta es la línea modificada
            float newScale = Mathf.Clamp(gameBoard.transform.localScale.x * scaleChange, minScale, maxScale);

            // Calcular la diferencia de escala
            float scaleDifference = newScale / gameBoard.transform.localScale.x;

            // Aplicar la nueva escala
            gameBoard.transform.localScale = new Vector3(newScale, newScale, gameBoard.transform.localScale.z);

            // Calcular cómo se ha movido el punto de interés debido al cambio de escala
            Vector3 newPinchCenterWorld = cam.ScreenToWorldPoint(center);
            newPinchCenterWorld.z = gameBoard.transform.position.z;

            // Calcular el vector de movimiento y ajustar la posición del gameBoard
            Vector3 movement = (pinchCenterWorld - newPinchCenterWorld) * (1 - scaleDifference);
            gameBoard.transform.position += movement;
        }
    }
